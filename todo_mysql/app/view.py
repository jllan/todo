...
def login():
	...
	if request.method == "POST":
		...
		return render_template('signin.html',myname = myname)
...
def register():
	...
	if request.method == "POST":
		...
		return render_template('signup.html',myname = myname)
...
def add_duty():
	if 'user_id' not in session:
		...
		if request.method == "POST":
			...
			category_list = Category.query.order_by(Category.category_id).all()
			return render_template('duty_add.html', category_list = category_list)
...
def my_duty():
	if 'user_id' not in session:
		...
		duty_list = db.session.execute(sql).fetchall()
		category_list = Category.query.order_by(Category.category_id).all()
		return render_template('duty.html', duty_list = duty_list, category_list=category_list, myname=myname)
